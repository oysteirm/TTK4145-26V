...
-----------------------------------
Functionallity: 
	- MessageSyncServer recieves system data from other elevators from UDP bcast and elevator from the elevator FSM.
	- It also UDP bcast it's own systemdata. It does not recieve it's own bcasts.
	- Every state and requests og the whole system is stores in the SystemData_t struct.
	- Every elevator state and requests have a list (barrier) with the elevators who have seen this information,
...
The Utilities:
	Update functions: 
		...
		- Uses ID, barriers and cyclic counter (CC) logic for to decide what data to update.
	Helper functions:
		-Converting functoins for peers.
		-Union and check for barriers.

-----------------------------------
map over SystemData_t / data that is being syncronized
	ID

	Elevator States:
	   [N_ELEVATORS]{ID,	IsAlive,	IS_Functinal,	Floor,	EB,		MD,		E_Barrier, 	Cab_Requests[N_FLOORS},
...
// Datatype for counter value with a corresponding barrier 
